...
unsigned int RenderMode = ENVMAP;

unsigned int zbuffer[ScreenSize];
...
static inline void drawpoint(point2d point, unsigned short *vram)
{
    if (point.x>=0 && point.x<ScreenWidth && point.y>=0 && point.y<ScreenHeight)
		*(vram + point.x + point.y * ScreenWidth) = 0xFFFF;
}
...
static inline void drawline (line2d line, unsigned short *vram)
{
	...
	if (abs(dy) < abs(dx))
	{
		...
		for (x=x1; x<x2; x++)
		{
			vramofs = ((y00 += l)>>fp)*ScreenWidth + x;
            if (vramofs>=0 && vramofs<ScreenSize) *(vram + vramofs) = c;
		}
	}
	else
	{
		...
		for (y=y1; y<y2; y++)
		{
			vramofs = y*ScreenWidth + ((x00 += l)>>fp);
            if (vramofs>=0 && vramofs<ScreenSize)  * (vram + vramofs) = c;
		}
...
if ((x0<-mlk || x0>ScreenWidth+mlk || y0<-mlk || y0>ScreenHeight+mlk) && 
    (x1<-mlk || x1>ScreenWidth+mlk || y1<-mlk || y1>ScreenHeight+mlk) && 
...
int sx1, sx2;
    ...
    for (y = y0; y<y1; y++)
    {
        ...
        for (x = sx1; x<sx2; x++)
        {
            if (vramofs>=0 && vramofs<ScreenSize && x>=0 && x<ScreenWidth)
                *(vram+vramofs)=c;
            vramofs++;
        }
        x01+=lx01;
...
int sz1, sz2;
    ...
    for (y = y0; y<y1; y++)
    {
        ...
        {
            sz1+=dz;
            if ((vramofs>=0 && vramofs<ScreenSize && x>=0 && x<ScreenWidth) && sz1<zbuffer[vramofs])
            {
                zbuffer[vramofs] = sz1;
                *(vram+vramofs)=c;
            }
...
{
    memset(zbuffer, 0xFFFFFF, sizeof(unsigned int) * ScreenSize);
}
...
void Render(object3d *obj, unsigned short shade[], unsigned short texture[], unsigned short texture_size, unsigned short *vram)
{
    ...
    int tu, tv, s;
    int texshr = (int)(log(256 / texture_size)/ log(2));
    
    int rmode;
    if (zb==1 && RenderMode>=FLAT) rmode = RenderMode | ZBUFFER;
        else rmode = RenderMode;
	...
	case POINTS:
		for (i=obj->npts-1; i>=0; i--)
            if (spts[i].x>=0 && spts[i].x<ScreenWidth && spts[i].y>=0 && spts[i].y<ScreenHeight)
                *(vram + spts[i].x + spts[i].y * ScreenWidth) = 0xFFFF;
	break;
	...
	case FLAT:
        ...
        for (i=0; i<obj->npls; i++)
        {
            ...
            if (n<0)
            {
                ...
                DrawFlatTriangle(obj->poly[j], vram, shade);
            }
        }
	break;
	...
	case (FLAT | ZBUFFER):
        ...
        for (i=obj->npls-1; i>=0; i--)
        {
            ...
            if (norms[i].z>=0)
                DrawFlatTriangleZB(obj->poly[i], vram, shade);
        }
	break;
	...
	case GOURAUD:
        ...
        for (i=0; i<obj->npls; i++)
        {
            ...
            n = vx0 * vy1 - vx1 * vy0;
            if (n<0)
                DrawGouraudTriangle(obj->poly[j], vram, shade);
        }
    break;
	...
	case (GOURAUD | ZBUFFER):
        ...
        {
            if (norms[i].z>=0)
                DrawGouraudTriangleZB(obj->poly[i], vram, shade);
        }
    break;
	...
	case (TEXTURE):
        ...
        for (i=0; i<obj->npls; i++)
        {
            ...
            if (n<0)
            {
                DrawTextureTriangle(obj->poly[j], vram, texshr, texture);
            }
        }
    break;
	...
	case (TEXTURE | ZBUFFER):
        ...
        {
            if (norms[i].z>=0)
                DrawTextureTriangleZB(obj->poly[i], vram, 0, texture);
        }
    break;
	...
	case (ENVMAP):
        ...
        for (i=0; i<obj->npls; i++)
        {
            ...
            n = vx0 * vy1 - vx1 * vy0;
            if (n<0)
                DrawEnvmappedTriangle(obj->poly[j], vram, texshr, texture);
        }
    break;
	...
	case (ENVMAP | ZBUFFER):
        ...
        {
            if (norms[i].z>=0)
                DrawEnvmappedTriangleZB(obj->poly[i], vram, 2, texture);
        }
    break;
